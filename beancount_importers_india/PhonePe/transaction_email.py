...
class PhonePeTransactionEmailImporter(importer.ImporterProtocol):
    def __init__(self,
        accounts_map_from_emails={'XXXXXXXXXX56':'Assets:Savings:Canara'}, # Copy the account number exactly from the email and map it to the account in your chart of accounts
        add_payee:bool = True,
        ):
    ...
    def extract_transaction_from_html(self, file:_FileMemo)->data.Transaction:
        payee_regex = re.compile(r'(Paid to|Received from)\s*((\w+ {1,2})*)') # payee can have multiple words separated by max 2 spaces
        narration_regex = re.compile(r'(Message\s*:\s*)((\w+ {1,2})*)') # narration can have multiple words separated by max 2 spaces
        ...
        payee, narration, account, reference, bank_reference, amount, date = None, None, None, None, None, None, None
        ...
        if date is None and (match:=date_regex.search(text)):
            try:
                ...
                raise ValueError(f'Could not parse the month from the date {match.groupdict()["month"]}')
            date = dateparse(f"{match.groupdict()['year']}-{month}-{match.groupdict()['day']}").date()
        ...
        if match:=amount_regex.search(text):
            amount = data.Amount(data.D(match.groupdict()['amount']), 'INR')

        if match:=account_regex.findall(text):
            account = self.accounts_map_from_emails.get(match[0], None)
            if account is None:
                raise ValueError(f'Account \'{match[0][1]}\' not found in the accounts_map_from_emails. available accounts are {self.accounts_map_from_emails.keys()}')
        ...
        assert date and amount and account, f'Could not extract date, amount or account from the file {file.name}. Date: {date}, Amount: {amount}, Account: {account}'
        assert status.lower() == 'successful', f'Transaction status is not success. Status: {status}'
        ...
        txn = data.Transaction(
            ...
            links=set(),
            postings=[
                data.Posting(account=account, units=amount, cost=None, price=None, flag=None, meta={"transaction_ref": bank_reference, "phonepe_txn_id": reference})
            ]
        )
        return txn
    ...
    def identify(self, f):
        # skip non pdf files
        if mimetypes.guess_type(f.name)[0] != 'text/html': return False
...
if __name__ == '__main__':
    ...
    importer = PhonePeTransactionEmailImporter(accounts_map_from_emails={
        'XXXXXXXXXX56':'Assets:Savings:ICICI',
        'XXXXXXXXXX08156':'Assets:Savings:Canara',
        'XXXXXX9927': 'Assets:Savings:HDFC'
    })
    for path in paths:
        print(f"Extracted transaction: {importer.extract_transaction_from_html(_FileMemo(path))}")
